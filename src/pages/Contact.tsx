            ...
            {contactMethods.map((method, index) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="text-xl font-bold text-slate-900 mb-4 group-hover:text-blue-600 transition-colors duration-200 text-center">
                    {method.title}
                  </h3>
                  <p className="text-slate-600 text-center mb-4 leading-relaxed">{method.description}</p>
                  <p className="font-semibold text-center text-sm text-blue-600 group-hover:text-blue-800 transition-colors duration-200">
                    {method.contact}
                  </p>
                </a>
              );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
              <div className="flex items-center mb-6">
                ...
                <h2 className="text-2xl font-bold text-slate-900">{t('contact.hours.title')}</h2>
              </div>
              <div className="space-y-4">
                {businessHours.map((schedule, index) => (
                  <div key={index} className="flex justify-between items-center py-4 border-b border-slate-100 last:border-b-0 group hover:bg-slate-50 rounded-lg px-2 transition-colors duration-200">
            ...
            <div className="bg-white rounded-2xl p-8 shadow-lg border border-slate-100">
              <div className="flex items-center mb-6">
                ...
                <h2 className="text-2xl font-bold text-slate-900">{t('contact.serviceArea.title')}</h2>
              </div>
              <div className="grid grid-cols-2 gap-3 mb-6">
                {serviceAreas.map((area, index) => (
                  <div key={index} className="flex items-center space-x-2 p-3 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors duration-200 group">
                    <span className="text-lg">{area.icon}</span>
                    <div>
                      <p className="font-medium text-slate-900 group-hover:text-teal-600 transition-colors duration-200">
                        {area.area}
                      </p>
                      <p className="text-xs text-slate-500">{area.type}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section className="py-20 lg:py-32 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {faqs.map((faq, index) => (
              <div key={index} className="bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-slate-100 group">
                <div className="flex items-start space-x-4">
                  <div className="bg-gradient-to-br from-blue-100 to-purple-100 rounded-full p-3 flex-shrink-0 group-hover:scale-110 transition-transform duration-200">
