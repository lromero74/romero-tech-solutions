    ...
    return (
      <div className="text-center py-8">
        ...
        <h3 className="text-xl font-semibold text-green-600 mb-2">
          {t('payment.successful', undefined, 'Payment Successful!')}
        </h3>
        <p className="text-gray-600">
          {t('payment.processedMessage', { invoiceNumber }, 'Your payment for invoice {invoiceNumber} has been processed.')}
        </p>
      </div>
    );
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="bg-gray-50 rounded-lg p-4 mb-6">
        <div className="flex justify-between items-center">
          <span className="text-gray-700 font-medium">{t('payment.invoiceLabel', undefined, 'Invoice:')}</span>
          <span className="text-gray-900 font-semibold">{invoiceNumber}</span>
        </div>
        <div className="flex justify-between items-center mt-2 pt-2 border-t border-gray-200">
          <span className="text-gray-700 font-medium">{t('payment.amountDueLabel', undefined, 'Amount Due:')}</span>
      ...
      {errorMessage && (
        <div className="flex items-start gap-3 bg-red-50 border border-red-200 rounded-lg p-4">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-red-800">{t('payment.error', undefined, 'Payment Error')}</p>
      ...
      <div className="flex gap-3 pt-4 border-t border-gray-200">
        ...
        >
          {isProcessing ? (
            ...
            <span>{t('payment.payAmount', { amount: `$${amount.toFixed(2)}` }, 'Pay {amount}')}</span>
          )}
        </button>
      </div>

      <p className="text-xs text-gray-500 text-center">
        {t('payment.stripeSecurityMessage', undefined, 'Payments are securely processed by Stripe. Your card information is never stored on our servers.')}
      </p>
    </form>
  );
};
