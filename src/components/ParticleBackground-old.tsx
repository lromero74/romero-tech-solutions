...
const ParticleBackground: React.FC = () => {
  ...
  const [backgroundBlobs, setBackgroundBlobs] = React.useState([
    { x: Math.random() * window.innerWidth, y: Math.random() * window.innerHeight, vx: (Math.random() - 0.5) * 0.5, vy: (Math.random() - 0.5) * 0.5, opacity: 1, fadeDirection: 'stable' as 'in' | 'out' | 'stable' },
    { x: Math.random() * window.innerWidth, y: Math.random() * window.innerHeight, vx: (Math.random() - 0.5) * 0.5, vy: (Math.random() - 0.5) * 0.5, opacity: 1, fadeDirection: 'stable' as 'in' | 'out' | 'stable' },
    { x: Math.random() * window.innerWidth, y: Math.random() * window.innerHeight, vx: (Math.random() - 0.5) * 0.5, vy: (Math.random() - 0.5) * 0.5, opacity: 1, fadeDirection: 'stable' as 'in' | 'out' | 'stable' },
    { x: Math.random() * window.innerWidth, y: Math.random() * window.innerHeight, vx: (Math.random() - 0.5) * 0.5, vy: (Math.random() - 0.5) * 0.5, opacity: 1, fadeDirection: 'stable' as 'in' | 'out' | 'stable' },
    { x: Math.random() * window.innerWidth, y: Math.random() * window.innerHeight, vx: (Math.random() - 0.5) * 0.5, vy: (Math.random() - 0.5) * 0.5, opacity: 1, fadeDirection: 'stable' as 'in' | 'out' | 'stable' }
  ...
  const handleParticleGeneration = React.useCallback((clickX: number, clickY: number) => {
  ...
  // Removed unused wouldIntersect function


  // Removed unused generateSinglePath function and getTurnDirection helper
  ...
  React.useEffect(() => {
    const generateLightning = () => {
      // Don't generate lightning if tab is not visible
  ...
  React.useEffect(() => {
    ...
    const interval = setInterval(() => {
      // Don't generate shapes if tab is not visible
      if (!isTabVisible) return;
  ...
  React.useEffect(() => {
    const animate = () => {
      ...
      if (!isTabVisible) {
        // Still request next frame to keep the loop alive
      ...
      setBackgroundBlobs(prevBlobs => {
        return prevBlobs.map((blob) => {
          ...
          // Check if blob is moving off screen and start fading
          const margin = 200; // Start fading when this close to edge
          const isNearEdge = newX < -margin || newX > window.innerWidth + margin || 
                           newY < -margin || newY > window.innerHeight + margin;
          
          if (isNearEdge && blob.fadeDirection === 'stable') {
