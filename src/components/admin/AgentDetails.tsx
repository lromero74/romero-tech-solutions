...
import {
  ...
  Link, Unlink, RotateCcw, Save, Plus, Trash2, Play, Heart, LineChart
} from 'lucide-react';
...
import { CurrentMetrics, SystemEventLogs, DiskHealthStatus, OSPatchStatus, PackageManagerStatus, HardwareTemperature, NetworkConnectivity, SecurityStatus, FailedLoginAttempts, ServiceMonitoring, OSEndOfLifeStatus, CommandDetailsDialog, HealthChecksTab, TrendsTab } from './agent-details';
...
const AgentDetails: React.FC<AgentDetailsProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'alerts' | 'commands' | 'inventory' | 'policies' | 'health' | 'trends'>('overview');
  ...
  return (
    <div className="space-y-6">
      ...
      {/* OS Patch Status */}
      <OSPatchStatus latestMetrics={latestMetrics} agentId={agentId} agent={agent} commands={commands} />
      ...
      <div className={`${themeClasses.bg.card} ${themeClasses.shadow.md} rounded-lg overflow-hidden`}>
        <div className="flex border-b border-gray-200 dark:border-gray-700">
          ...
          >
            ...
            {t('agentDetails.tabs.trends', undefined, 'Trends')}
          </button>
        </div>
